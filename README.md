...
Lowest total after 10 hands wins the set. Ties break by most hands won, then most recent hand won (Big 2 has no official governing rules; this is the common convention among digital implementations). The winner of each hand leads the next one. Set scores persist in localStorage, so you can close the app mid-set and pick it back up.
...
In a 250-hand simulation, hard averages 38 penalty points per set against medium's 52 and easy's 57.
